...
export default function QuizPlay() {
  const [questions, setQuestions] = useState(["What is JavaScript?", "What is closure?", "What is DOM?"]);
  ...
  // };
    ...
    //  navigate("/scoreboard");
     // 1) Show the prompt
     setShowTopicPrompt(true);
     // 2) Save scores now, but do NOT navigate yet
     localStorage.setItem("boysScore", boysScore);
     localStorage.setItem("girlsScore", girlsScore);
    }
  };
...
const loadNewTopic = (topic) => {
  // Here you'd fetch new topic-specific questions (dummy below)
  const newQuestions = [`Question 1 about ${topic}`, `Question 2 about ${topic}`, `Question 3 about ${topic}`];
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
        <div className="bg-white p-6 rounded-xl shadow-lg text-center space-y-4">
          <h3 className="text-xl font-bold">
            Do you want to continue with another topic?
          </h3>
    ...
    <div className="p-6 bg-yellow-50 min-h-screen flex flex-col justify-between">
      <div className="text-center">
        ...
        >
          {questions.length === 0 ? (
            ...
            <h2 className="text-xl font-bold mb-4">{questions[currentQuestionIndex]}</h2>
          )}
      ...
      <div className="flex justify-around mt-10">
        <TeamScore
          team="boys"
          score={boysScore}
          setScore={setBoysScore}
          img={boysImg}
        />
        <TeamScore
          team="girls"
          score={girlsScore}
          setScore={setGirlsScore}
          img={girlImg}
        />
      </div>
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      <div className="flex gap-2">
        ...
        <button
          onClick={() => setScore((prev) => Math.max(prev - 1, 0))} // ✅ No negative score
          className="bg-red-600 text-white px-4 py-2 rounded"
        >
          -1
        </button>
